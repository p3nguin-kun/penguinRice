...
[ -d ~/penguinDotfiles-artix ] && rm -rf ~/penguinDotfiles-artixrunit
[ -d ~/penguinFox-Librewolf ] && rm -rf ~/penguinFox-Librewolf
printf "Cloning rice from https://github.com/p3nguin-kun/penguinDotfiles-artix\n"
cd
git clone --depth=1 https://github.com/p3nguin-kun/penguinDotfiles-artix.git
...
for archivos in ~/penguinDotfiles-artixrunit/config/*; do
...
for archivos in ~/penguinDotfiles-artixrunit/themes/*; do
...
for archivos in ~/penguinDotfiles-artixrunit/lightdm-config/*; do
...
for archivos in ~/penguinDotfiles-artixrunit/wallpapers; do
	cp -R "${archivos}" ~/
	if [ $? -eq 0 ]; then
		printf "%s%s%s folder copied succesfully!%s\n" "${BLD}" "${CGR}" "${archivos}" "${CNC}"
	else
...
for archivos in ~/penguinDotfiles-artixrunit/misc/applications/*; do
...
if ls /run/runit/service/ | grep -E 'gdm|lightdm|lxdm|lxdm-gtk3|sddm|slim|xdm'; then
	echo "Disabling currently enabled display manager"
	sudo rm /run/runit/service/$(ls /run/runit/service/ | grep -E 'gdm|lightdm|lxdm|lxdm-gtk3|sddm|slim|xdm')
fi

echo "Enabling LightDM"
sudo ln -s /etc/sv/lightdm /run/runit/service/
printf "%s%sDone!%s\n\n" "${BLD}" "${CGR}" "${CNC}"
sleep 1
clear
...
sudo ln -s /etc/sv/NetworkManager /run/runit/service
printf "%s%sDone!%s\n\n" "${BLD}" "${CGR}" "${CNC}"
sleep 1
clear
